...
function recipeToolFilter(recipe, tools) {
  // The recipes now come from the vendor plugins and name the VENDOR's own
  // tool surface (Shell/ReadFile/…, run_terminal_cmd/read_file/…,
  // exec_command/apply_patch/…). Those names are registered in the kernel
  // preset's scope, not in the host's global registry, so we must NOT filter
  // them against knownGlobalToolNames (that check was for the old distilled
  // recipes that named stock DSH tools). tools.restrict() on the child
  // validates against the child's inherited surface, which includes the
  // vendor tools when the parent runs on the kernel preset.
  const extra = availableSearchTools()
  return { allow: recipe.toolFilter.allow.concat(extra) }
}
...
function loadCredentials() {
  const creds = { kimi: '', kimiOAuth: false, grok: '', codex: '', codexBase: 'https://ollama.com/v1', codexModel: 'deepseek-v4-flash:0731', minimax: '', ollamaKey: '', opencodeGoKey: '' }
...
async function apply(ctx) {
    ...
    // Test/fallback switch: when set, route kimi/minimax through the user's
    // ollama subscription and codex through opencode-go (gpt-5.6-luna), so the
    // kernels stay testable when their own APIs are out of quota.
    // Default ON (the kernel APIs are out of quota in this environment);
    // set DSH_KERNEL_USE_FALLBACK=0 to force the official kernel APIs.
    const useFallback = process.env.DSH_KERNEL_USE_FALLBACK !== '0'
    ...
    ctx.on('agent/created', ({ agent }) => {
      ...
      try {
        ...
        // deny every inherited tool (parent preset + host). DSH's restrict()
        // filters inherited tools but never a scope's own registrations, so the
        // whitelist controls the vendor surface and `allow: []` clears the rest.
    ...
    if (useFallback && creds.ollamaKey) {
      ...
      llm.registerAdapter(['grok-kernel'], makeResponsesAdapter({
        ...
        models: GROK_MODELS, proxy: PROXY, attachments, defaultEffort: 'xhigh',
      ...
      tools.register(toolDef({
        ...
        parameters: {
          ...
          properties: {
            ...
            effort: { type: 'string', description: 'Optional reasoning-effort override. grok: low|medium|high|xhigh (default xhigh); kimi: low|medium|high|max (default max); codex: none|high.' },
        ...
        execute: async (args, exec) => {
          ...
          const KERNEL_DEFAULTS = {
            grok: { model: useFallback ? 'gpt-oss:120b' : 'grok-4.6', effort: useFallback ? 'high' : 'xhigh' },
            ...
            minimax: { model: useFallback ? 'minimax-m3' : 'MiniMax-M2.7', effort: undefined },
          }
        ...
        tools.register(toolDef({
          ...
          execute: async (args, exec) => {
            const map = {
              ...
              minimax: { provider: 'minimax-kernel', model: useFallback ? 'minimax-m3' : 'MiniMax-M2.7' },
