...
// kimi-cli honors KIMI_CODE_HOME to relocate its state dir (default ~/.kimi-code).
...
// Default to a DIRECT connection; a hardcoded local proxy port breaks machines
// that do not run one.
...
function kimiDeviceId() {
  const raw = readText(path.join(KIMI_CODE_HOME, 'device_id')).trim()
  return raw || 'dsh-kernel-mesh'
}

// Kimi Code CLI 0.39.1 wire values for the /search and /fetch service calls
// (Bearer auth IS correct here — only /messages uses x-api-key).
function kimiServiceHeaders(extra) {
  let deviceName = 'dsh-kernel-mesh'
  ...
  return Object.assign({
    'user-agent': 'kimi-code-cli/0.39.1 (dsh-kernel-mesh)',
...
function loadKimiBearer() {
  try {
    const t = JSON.parse(readText(path.join(KIMI_CODE_HOME, 'credentials', 'kimi-code.json')))
    if (t && typeof t.access_token === 'string' && t.access_token) return t.access_token
  } catch {}
  try {
    const cfg = readText(path.join(KIMI_CODE_HOME, 'config.toml'))
...
function loadGrokKey() {
  try {
    const auth = JSON.parse(readText(path.join(GROK_HOME, 'auth.json')))
...
// Optional async key provider injected by index.js (freshGrokKey — refreshes
// expired JWTs via OIDC before returning). Without it the raw file read above
// is the fallback.
let grokKeyProvider = null
function setGrokKeyProvider(fn) { grokKeyProvider = (typeof fn === 'function') ? fn : null }
async function grokKey() { return grokKeyProvider ? await grokKeyProvider() : loadGrokKey() }
...
function grokSearchHeaders(key) {
  return {
    ...
    'user-agent': 'dsh-kernel-mesh/0.1.4',
  }
}
