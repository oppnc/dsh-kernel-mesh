...
async function apply(ctx) {
    ...
    // kimi-cli keeps its model catalog in the user's config.toml; the docs
    // example confirms kimi-for-coding max_context_size = 262144. k3 / k3-256k
    // are OAuth-managed models (refreshed from the provider's /models endpoint),
    // so their windows are the published values rather than source constants.
    const KIMI_MODELS = [
    ...
    const CODEX_MODELS = creds.codexModel
      ...
      : []
    // Mini-Agent source default model is MiniMax-M2.5 (config-example.yaml /
    // config.py) and its Anthropic client sends max_tokens: 16384. The source
    // does not publish a context-window number, so 204800 stays as the known
    // MiniMax M2 window.
    const MINIMAX_MODELS = [
      { id: 'MiniMax-M2.5', name: 'MiniMax M2.5 (Mini-Agent kernel)', contextWindow: 204800, defaultMaxTokens: 16384 },
    ]
